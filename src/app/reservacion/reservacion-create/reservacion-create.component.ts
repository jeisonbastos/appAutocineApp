...
import { IProductoClasificacion } from '../reservacion-clasificacion';
import { IProductoTipo } from '../reservacion-tipo';
...
export class ProductoCreateComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.gService.create('reservacion', this.createForm.value).subscribe(
      (respuesta: any) => {
        (this.reservacion = respuesta), this.router.navigate(['productos/lista']);
      },
      (error: any) => {
        this.error = error;
  ...
  getClasificaciones() {
    this.gService
      .list('clasificacion reservacion/all')
